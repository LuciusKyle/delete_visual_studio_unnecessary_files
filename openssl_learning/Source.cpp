...
{
	std::string str;
	Md5FileCpp(std::string(R"(C:\Users\Lucius\Desktop\test\170928-BLACKED.mp4)"), str);
...
bool Md5FileCpp(const std::string& filepath, std::string& strMd5)
{
	...
	SHA_CTX *sha1_ctx = new SHA_CTX;
	MD5_CTX ctx;
	unsigned char *buffer = new unsigned char[my_buffer_lenth]{ 0 };
	size_t len = 0;
	unsigned char md[16];
	unsigned char sh[33]{ 0 };
	unsigned char sh1[SHA_DIGEST_LENGTH]{ 0 };
	MD5_Init(&ctx);
	SHA256_Init(sha_ctx);
	SHA1_Init(sha1_ctx);
	while ((len = fread(buffer, 1, my_buffer_lenth, pFile)) > 0) {
